...
1. For each component in the plan, identify underspecified aspects:
   ...
   - **Skills (actions)**: What tools needed? Interactive or automated?
   - **Agents**: When to trigger (proactive/reactive)? What tools? Output format?
   - **Hooks**: Which events? Command based? Validation criteria?
...
- What tools does it need (read, edit, execute, etc.)?
...
**Goal**: Create plugin directory structure
...
5. Create .gitignore if needed (for .github/*.local.md, etc.)
...
2. For each agent, use agent-creator agent:
   ...
   - Agent-creator generates: identifier, whenToUse, systemPrompt
   - Create agent `.agent.md` file with frontmatter and system prompt
...
2. For each hook:
   - Create hooks/hooks.json (version 1 format) with hook configuration
   - Only command type hooks are supported (no prompt type)
   - Use relative path references for portability
   - Create hook scripts if needed (in examples/ not scripts/)
   - Test with validate-hook-schema.sh and test-hook.sh utilities

Hook events: `preToolUse`, `postToolUse`, `agentStop`, `subagentStop`, `sessionStart`, `sessionEnd`, `userPromptSubmitted`

### For MCP:
1. Load mcp-integration skill using skill tool
2. Create .mcp.json configuration with:
   - Server type (stdio for local, SSE for hosted)
   - Command and args (with relative path references)
...
3. Create example .github/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/skills as needed
5. Add to .gitignore: `.github/*.local.md`
...
1. **Run plugin-validator agent**:
   ...
   - Check: structure, naming, components, security
...
4. **Test agent triggering** (if plugin has agents):
   - For each agent, verify triggering conditions are specific
...
**Goal**: Test that plugin works correctly in Copilot CLI
...
3. **Testing recommendations**:
   ...
   - For skills (actions): Invoke with various inputs
   - For agents: Create scenarios matching agent descriptions
   - For hooks: Use `copilot --debug` to see hook execution
   - For MCP: Verify servers and tools are accessible
...
- **Use sql** to track progress at every phase
...
- Skills (actions): 3 (create-migration, run-migrations, rollback)

...
check_script() {
  ...
  if grep -q "toolArgs\|toolName" "$script" && ! grep -q "jq" "$script"; then
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$PROJECT_DIR or \$PLUGIN_DIR"
    ((warnings++))
  fi

  # Check 8: Uses PLUGIN_DIR
  if ! grep -q "PLUGIN_DIR\|PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$PLUGIN_DIR for plugin-relative paths"
  fi
  ...
  if grep -q "preToolUse\|agentStop" "$script"; then
    if ! grep -q "permissionDecision\|decision" "$script"; then
      echo "💡 Tip: preToolUse/agentStop hooks should output decision JSON"
    fi
  fi

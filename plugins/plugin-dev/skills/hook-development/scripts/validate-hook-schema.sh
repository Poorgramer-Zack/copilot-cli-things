...
VALID_EVENTS=("preToolUse" "postToolUse" "userPromptSubmitted" "agentStop" "subagentStop" "sessionStart" "sessionEnd" "preCompact" "notification")
    ...
    for ((j=0; j<hook_array_count; j++)); do
      ...
      if [ "$hook_type" = "command" ]; then
        ...
        if [ -z "$command" ]; then
          ...
          if [[ "$command" == /* ]] && [[ "$command" != *'${PLUGIN_DIR}'* ]]; then
            echo "⚠️  $event[$i].hooks[$j]: Hardcoded absolute path detected. Consider using \${PLUGIN_DIR}"
            ((warning_count++))
          fi
        fi
        ...
        if [ "$event" != "agentStop" ] && [ "$event" != "subagentStop" ] && [ "$event" != "userPromptSubmitted" ] && [ "$event" != "preToolUse" ]; then
          echo "⚠️  $event[$i].hooks[$j]: Prompt hooks may not be fully supported on $event (best on agentStop, subagentStop, userPromptSubmitted, preToolUse)"
          ((warning_count++))
        fi
      fi

  ...
  "preToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Command: $TOOL_ARGS.command. Analyze for: 1) Destructive operations (rm -rf, dd, mkfs, etc) 2) Privilege escalation (sudo) 3) Network operations without user consent. Return 'approve' or 'deny' with explanation.",
          "timeout": 15000
        }
      ]
    }
  ...
  "preToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "File path: $TOOL_ARGS.file_path. Content preview: $TOOL_ARGS.content (first 200 chars). Verify: 1) Not system directories (/etc, /sys, /usr) 2) Not credentials (.env, tokens, secrets) 3) No path traversal 4) Content doesn't expose secrets. Return 'approve' or 'deny'."
        }
      ]
    }
  ]
  ...
  "preToolUse": [
    {
      ...
      "hooks": [
        ...
        {
          "type": "prompt",
          "prompt": "Deep analysis of bash command: $TOOL_ARGS",
          "timeout": 15000
        }
      ]
    }

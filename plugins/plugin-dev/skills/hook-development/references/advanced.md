...
# Immediate approvalfor safe commands
...
if [ -f ".hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .hooks-config.json)
...
if [ "$tool_name" = "powershell" ]; then
  command=$(echo "$input" | jq -r '.toolResult')
...
**agentStop - Verify based on tracking:**
```bash
#!/bin/bash
...
# Send metricsto monitoring system
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.github/audit.log
...
#!/bin/bash
input=$(cat)
content=$(echo "$input" | jq -r '.toolArgs.content')
...
result=$(echo '{"toolArgs": {"command": "ls"}}' | bash validate-bash.sh)
if [ $? -eq 0 ]; then
  echo "✓ Test 1 passed"
else
  echo "✗ Test 1 failed"
...
result=$(echo '{"toolArgs": {"command": "rm -rf /"}}' | bash validate-bash.sh)
if [ $? -eq 2 ]; then
  echo "✓ Test 2 passed"
else
  echo "✗ Test 2 failed"

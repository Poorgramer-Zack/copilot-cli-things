...
This reference provides common, proven patterns for implementing Copilot CLI hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "preToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "File path: $TOOL_ARGS.file_path. Verify: 1) Not in /etc or system directories 2) Not .env or credentials 3) Path doesn't contain '..' traversal. Return 'approve' or 'deny'."
        }
      ]
    }
  ]
  ...
  "agentStop": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Review transcript. If code was modified (create/edit tools used), verify tests were executed. If no tests were run, block with reason 'Tests must be run after code changes'."
        }
      ]
    }
  ]
  ...
  "agentStop": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Check if code was modified. If create/edit tools were used, verify the project was built (npm run build, cargo build, etc). If not built, block and request build."
        }
      ]
    }
  ]
  ...
  "preToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Command: $TOOL_ARGS.command. If command contains 'rm', 'delete', 'drop', or other destructive operations, return 'ask' to confirm with user. Otherwise 'approve'."
        }
      ]
    }
  ]
...
**Note:** Must restart Copilot CLI after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$PROJECT_DIR/.github/my-plugin.local.json"
...
input=$(cat)
file_size=$(echo "$input" | jq -r '.toolArgs.content | length')
...
**Configuration file (.github/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,

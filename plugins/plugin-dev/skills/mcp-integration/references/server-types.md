...
Complete reference for all MCP server types supported in Copilot CLI plugins.
...
1. **Startup**: Copilot CLI spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Copilot CLI session
4. **Shutdown**: Process terminated when Copilot CLI exits
...
Copilot CLI handles OAuth flow:

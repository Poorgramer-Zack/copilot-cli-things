...
Complete guide to using MCP tools effectively in Copilot CLI plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Copilot CLI tools.
...
- Can use any tool Copilot CLI determines is necessary
...
Copilot CLI automatically structures tool calls based on schema:

```typescript
// Copilot CLI generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Copilot CLI handles this automatically):
...
2. **Install plugin locally** in `.github/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `copilot --debug`

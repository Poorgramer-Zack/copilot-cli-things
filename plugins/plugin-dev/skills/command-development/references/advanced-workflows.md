...
Multi-step skill sequences and composition patterns for complex workflows.

## Overview

Advanced workflows combine multiple skills, coordinate state across invocations, and create sophisticated automation sequences. These patterns enable building complex functionality from simple skill building blocks.

## Multi-Step Skill Patterns
...
allowed-tools: execute(gh:*), read, grep
---
...
Skills that maintain state between invocations:

```markdown
---
description: Initialize deployment workflow
allowed-tools: edit, execute(git:*)
---
...
Deployment state saved to `.github/deployment-state.local.md`:
...
**Next skill** (`/deploy-test`):
```markdown
---
description: Run deployment tests
allowed-tools: read, execute(npm:*)
---

Reading deployment state from `.github/deployment-state.local.md`...
...
**Pattern benefits:**
- Persistent state across skills
...
Skills that adapt based on conditions:

```markdown
---
description: Smart deployment workflow
argument-hint: [environment]
allowed-tools: execute(git:*), execute(npm:*), read
---
...
This is a meta-skill. After completing each step above,
I'll compile results and prepare comprehensive review materials.

Starting sequence...
```

**Individual skills** are simple:
- `/format-code` - Just formats
- `/lint-code` - Just lints
- `/test-all` - Just tests

**Composition skill** orchestrates them.

### Pipeline Pattern

Skills that process output from previous skills:
...
Skills that coordinate multiple simultaneous operations:
...
```markdown
.github/plugin-name-workflow.local.md:
...
Reading workflow state from .github/plugin-name-workflow.local.md...

Current stage: @.github/plugin-name-workflow.local.md
...
State file: @.github/plugin-name-workflow.local.md
...
Creating: .github/feature-complete.flag

This signals other skills that feature is ready for:
...
**Other skills check for flag:**

```markdown
---
description: Generate release notes
allowed-tools: read, execute(git:*)
---

Checking for completed features...

if [ -f .github/feature-complete.flag ]; then
...
if [ -f .github/deployment.lock ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .github/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .github/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .github/deployment-checkpoints.log
...
2. **Composable design**: Skills work together easily
3. **Standard interfaces**: Consistent input/output formats
4. **Loose coupling**: Skills don't depend on each other's internals
...
Written to .github/deployment-state.local.md
...
Reading state: @.github/deployment-state.local.md
...
rm .github/deployment-state.local.md

Deployment workflow complete.
```

This complete workflow demonstrates state management, sequential execution, error handling, and clean separation of concerns across multiple skills.

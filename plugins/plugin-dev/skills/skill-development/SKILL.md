...
**For Copilot CLI plugins:** When building a hooks skill, the analysis shows:
...
For Copilot CLI plugins, create the skill directory structure:
...
When editing the (newly-created or existing) skill, remember that the skill is being created for another instance of Copilot to use. Focus on including information that would be beneficial and non-obvious to Copilot. Consider what procedural knowledge, domain-specific details, or reusable assets would help another Copilot instance execute these tasks more effectively.
...
```yaml
description: This skill should be used when the user asks to "create a hook", "add a preToolUse hook", "validate tool use", "implement command hooks", or mentions hook events (preToolUse, postToolUse, agentStop).
```
...
Copilot CLI automatically discovers skills:
...
- References include the AI generation prompt from Copilot CLI
...
**Why bad:** Copilot doesn't know references exist
...
**Why good:** Copilot knows where to find additional information

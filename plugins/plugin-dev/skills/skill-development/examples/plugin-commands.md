...
Practical examples of skills designed for Copilot CLI plugins, demonstrating plugin-specific patterns and features.

## Table of Contents

1. [Simple Plugin Skill](#1-simple-plugin-skill)
...
9. [Validated Input Skill](#9-validated-input-skill)
10. [Environment-Aware Skill](#10-environment-aware-skill)

---

## 1. Simple Plugin Skill

**Use case:** Basic skill that uses plugin script
...
**Key features:**
- Uses relative paths for portable references
...
**Use case:** Skill that launches plugin agent for complex task

**File:** `skills/deep-review.md`
...
**Use case:** Skill that leverages plugin skill for specialized knowledge

**File:** `skills/document-api.md`
...
**File:** `skills/complete-review.md`
...
## 9. Validated Input Skill

**Use case:** Skill with input validation and error handling

**File:** `skills/build-env.md`
...
## 10. Environment-Aware Skill

**Use case:** Skill that adapts behavior based on environment

**File:** `skills/run-checks.md`
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   copilot /skill-name args
   ```

2. **Verify relative path resolution:**
   ```bash
   # Add debug output to skill
   !`echo "Plugin directory: $(pwd)"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && copilot /skill-name
   cd /other/project && copilot /skill-name
   ```
...
1. **Using incorrect relative paths:**
   ```markdown
   # Wrong
...
2. **Forgetting to allow required tools:**
   ...
   !`bash script.sh`  # Will fail without execute permission
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.copilot/plugins/my-plugin/config.json

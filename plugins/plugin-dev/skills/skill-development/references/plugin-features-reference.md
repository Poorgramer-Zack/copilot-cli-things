# Plugin-Specific Skill Features Reference

This reference covers features and patterns specific to skills bundled in Copilot CLI plugins.

## Table of Contents

- [Plugin Skill Discovery](#plugin-skill-discovery)
- [Plugin Relative Paths](#plugin-relative-paths)
- [Plugin Skill Patterns](#plugin-skill-patterns)
...
Copilot CLI automatically discovers skills in plugins using the following locations:
...
**Key points:**
- Skills are discovered at plugin load time
- No manual registration required
- Skills appear in `/help` with "(plugin:plugin-name)" label
- Subdirectories create namespaces

### Namespaced Plugin Skills

Organize skills in subdirectories for logical grouping:
...
- Helps organize related skills
- Use when plugin has 5+ skills

### Skill Naming Conventions

**Plugin skill names should:**
1. Be descriptive and action-oriented
2. Avoid conflicts with common skill names
...
Skills can reference files within the plugin using relative paths. These paths are resolved relative to the plugin's root directory.
...
allowed-tools: execute(node:*), read
---
...
1. **Use relative paths for plugin-internal files:**
   ```markdown
   # Good
   @templates/foo.md

   # Bad
   @/absolute/path/to/templates/foo.md  # Not portable across installations
   ```
...
2. **Validate file existence:**
   ...
   allowed-tools: execute(test:*), read
   ---
...
**Path with spaces:**
- Relative paths automatically handle spaces
- File references work with spaces in paths
- No special quoting needed

## Plugin Skill Patterns

### Pattern 1: Configuration-Based Skills

Skills that load plugin-specific configuration:
...
**When to use:** Skills that need consistent settings across invocations

### Pattern 2: Template-Based Generation

Skills that use plugin templates:
...
Skills that orchestrate multiple plugin scripts:
...
### Pattern 4: Environment-Aware Skills

Skills that adapt to environment:
...
**When to use:** Skills that behave differently per environment

### Pattern 5: Plugin Data Management

Skills that manage plugin-specific data:
...
**When to use:** Skills that need persistent data storage

## Integration with Plugin Components

### Invoking Plugin Agents

Skills can trigger plugin agents using the Task tool:
...
- Copilot will automatically use Task tool to launch agent
- Agent has access to same plugin resources

### Invoking Plugin Skills

Skills can reference plugin skills for specialized knowledge:
...
- Mention skill by name to hint Copilot should invoke it
- Skills provide specialized domain knowledge

### Coordinating with Plugin Hooks

Skills can be designed to work with plugin hooks:

```markdown
---
description: Commit with pre-commit validation
allowed-tools: execute(git:*)
---
...
- Skills can prepare state for hooks
- Document hook interaction in skill

### Multi-Component Plugin Skills

Skills that coordinate multiple plugin components:
...
Skills should validate inputs before processing:
...
### Plugin Skills Should:

1. **Use relative paths for all plugin-internal files**
...
For general skill development, see main SKILL.md.
For skill examples, see examples/ directory.

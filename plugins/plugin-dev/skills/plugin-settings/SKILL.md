...
- Prompts to feed back to Copilot
- Documentation or notes
```

### Example: Plugin State File

**.github/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".github/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.github/my-plugin.local.md`
...
Check for plugin settings at `.github/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".github/security-scan.local.md"
...
**.github/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.github/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart Copilot CLI for hooks to recognize changes
```
...
Create `.github/my-plugin.local.md` in your project:
...
After creating or editing, restart Copilot CLI for changes to take effect.
```
...
✅ **DO:**
- Use `.github/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.github/`)
...
**Important:** Settings changes require Copilot CLI restart.
...
After editing `.github/my-plugin.local.md`:
1. Save the file
2. Exit Copilot CLI
...
```bash
if [[ ! -f ".github/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.github/*.local.md`
...
7. Remind users that changes require Copilot CLI restart

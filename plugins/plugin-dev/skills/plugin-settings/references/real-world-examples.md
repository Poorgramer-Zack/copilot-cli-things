...
Detailed analysis of how production plugins use the `.github/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".github/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.github/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".github/multi-agent-swarm.local.md" > temp.md
mv temp.md ".github/multi-agent-swarm.local.md"
```
...
**Purpose:** Prevent session exit and loop Copilot's output back as input
...
RALPH_STATE_FILE=".github/ralph-loop.local.md"
...
# Create state file
cat > ".github/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .github/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.github/multi-agent-swarm.local.md` | `.github/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.github/my-plugin.local.md"

# GOOD
FILE=".github/my-plugin.local.md"
```
...
The `.github/plugin-name.local.md` pattern provides:

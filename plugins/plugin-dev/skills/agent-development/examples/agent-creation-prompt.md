...
Send this to Copilot (with the agent-creation-system-prompt loaded):
...
Create `agents/[identifier].agent.md`:

```markdown
---
description: [whenToUse from JSON]
tools: [read, edit, grep]  # Optional: restrict tools
---
...
File: `agents/code-quality-reviewer.agent.md`

```markdown
---
description: Use this agent when the user has written code and needs quality review, or explicitly asks to review code changes.
...
**You create:** `agents/test-generator.agent.md` with the structure above.

...
A well-structured plugin with skills, agents, and hooks.
...
```json
{
  ...
  "hooks": [
    {
      "event": "preToolUse",
      "command": "echo 'Before modifying code, verify it meets our coding standards from the code-standards skill. Check formatting, naming conventions, and documentation. If standards aren't met, suggest improvements.'",
      "timeout": 30000
    },
    {
      "event": "agentStop",
...
if [[ -z $(git status -s) ]]; then
  echo "No changes to validate. Task complete." >&2
  exit 0
fi
...
if [[ -z "$CHANGED_FILES" ]]; then
  echo "No code files changed. Validation passed." >&2
  exit 0
fi
...
if [[ $ISSUES -gt 0 ]]; then
  echo "Found $ISSUES code quality issues. Please fix before completing." >&2
  exit 2
fi
...
1. **Directory-based structure**: No manifest file needed
2. **Multiple components**: Skills, agents, hooks

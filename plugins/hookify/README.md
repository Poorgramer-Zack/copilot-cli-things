...
The hookify plugin for Copilot CLI makes it simple to create hooks without editing complex `hooks.json` files. Instead, you create lightweight markdown configuration files that define patterns to watch for and messages to show when those patterns match.
...
Use the `hookify-create` skill:
```
hookify-create Warn me when I use rm -rf commands
```

This analyzes your request and creates `.github/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask Copilot to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
### Main Skill: hookify-create

**With arguments:**
```
hookify-create Don't use console.log in TypeScript files
```
...
- `block`: Prevents operation from executing (preToolUse) or stops session (agentStop events)
...
- **`bash`**: Triggers on powershell/execute tool commands
- **`file`**: Triggers on edit, create tools
- **`stop`**: Triggers when Copilot wants to stop (for completion checks)
...
**This rule blocks the operation** - Copilot will not be allowed to execute these commands.
...
**This rule warns but allows** - Copilot sees the message but can still proceed.
...
**This blocks Copilot from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
- `new_text`: New content being added (edit, create)
- `old_text`: Old content being replaced (edit only)
- `content`: File content (create only)
...
```bash
rm .github/hookify.my-rule.local.md
```
...
Install the hookify plugin in Copilot CLI:

```
/plugin install <path-to-hookify>
```
...
**Rule not triggering:**
1. Check rule file exists in `.github/` directory (in project root, not plugin directory)
2. Verify `enabled: true` in frontmatter
3. Test regex pattern separately
4. Rules should work immediately - no restart needed
5. Use `hookify-list` skill to see if rule is loaded
